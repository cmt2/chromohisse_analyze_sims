...
for (i in 1:num_slurms) {
    ...
    # change jobname
    this_template <- gsub("JOBNAME", paste0("chromosse_",i), this_template)
    ...
    # substitute job folder
    this_template <- gsub("chromohisse_jobs", "chromosse_jobs_low_mu", this_template)
    
    # write
    this_fn <- paste0("chromosse_slurms_low_mu/chromosse_jobs_",i,".sh")
